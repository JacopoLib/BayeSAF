function [x, p_x, x_nb, p_x_nb, AR, R_hat, t_convergence] = DifferentialEvolutionMarkovChain(Posterior_PDF, classes, LowerBound_x, UpperBound_x, n_ranges, LowerBound_eta_B_star, UpperBound_eta_B_star, maxIterations, t_burnin, scaling_factor, noise_X, noise_nc, noise_eta, N_chains, outlier_method)
...
% 3) x_nb    : ((t_convergence - t_burnin) x 3*Nc-1 x N_chains) array containing the
...
% 4) p_x_nb  : ((t_convergence - t_burnin) x N_chains) array containing the posterior density
...
x = nan(maxIterations,3*numComponents-1,N_chains); p_x = nan(maxIterations,N_chains); accept = nan(maxIterations,N_chains); AR = nan(maxIterations,N_chains); R_hat = 1e+18*ones(floor((maxIterations-2*t_burnin)/2), 3*numComponents-1);          % Preallocate memory for chains, density, acceptance rate, and R-hat statistic
...
for i = 1:N_chains
    ...
    for l = 2*numComponents:3*numComponents-1
        ...
        X(i,l) = eta_B_star_norm_l_values(findIndex_eta(classl, X(i,l-numComponents), X(i,l)));
    end

    p_X(i,t) = Posterior_PDF(X(i,1:numComponents-1),X(i,numComponents:2*numComponents-1),X(i,2*numComponents:3*numComponents-1));     % Compute density initial population

end

x(t,1:3*numComponents-1,1:N_chains) = reshape(X',1,3*numComponents-1,N_chains); p_x(t,1:N_chains) = p_X';   % Store initial states and density
...
while ~convergence && t <= maxIterations         % Dynamic part: evolution of N chains
    ...
    for i = 1:N_chains                                                      % Create proposals and accept/reject
        ...
        % Pick topochemical atom indices from database displaying the closest value to those sampled
        for j = 2*numComponents:3*numComponents-1
            classj = classes{j-2*numComponents+1};
            eta_B_star_norm_j_values = [classj.eta_B_star_norm];
            Xp(i,j) = eta_B_star_norm_j_values(findIndex_eta(classj, Xp(i,j-numComponents), Xp(i,j)));
        end
    end
    ...
    if t >= 2*t_burnin && mod(t,2) == 0
        ...
        if all(R_hat(counter_check,:) <= 1.2) % ---- Convergence is reached ---- %
            t_convergence = t;
            R_hat = R_hat(1:counter_check,:);
            AR = AR(1:t_convergence,:);
            fprintf(fid, ['R-statistic below the critical threshold of 1.2 for all parameters of the target distribution: convergence reached after ', num2str(t_convergence), ' iterations within each chain. These samples will be used for posterior analysis after discarding the burn-in period.\n']);
            convergence = true;
        end
