...
for l = 1:numComponents
    if strcmp(families(l), 'nparaffins')
        ...
        nc_idx = find(nparaffins.nC == nc_MAP(l));
        eta_B_star_diff = abs(nparaffins.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l));
        min_eta_B_star_diff = min(abs(nparaffins.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l)));
        ...
        species_MAP(l) = nparaffins_names.Name(nc_idx(1) + row_MAP_idx - 1);
        Wi_MAP(l) = nparaffins.W(nc_idx(1) + row_MAP_idx - 1);
    elseif strcmp(families(l), 'isoparaffins')
        ...
        nc_idx = find(isoparaffins.nC == nc_MAP(l));
        eta_B_star_diff = abs(isoparaffins.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l));
        min_eta_B_star_diff = min(abs(isoparaffins.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l)));
        row_MAP_idx = find(eta_B_star_diff == min_eta_B_star_diff, 1);

        names = 'isoparaffins/isoparaffins_Names.csv';
        isoparaffins_names = readtable(names, 'VariableNamingRule', 'preserve');

        species_MAP(l) = isoparaffins_names.Name(nc_idx(1) + row_MAP_idx - 1);
        Wi_MAP(l) = isoparaffins.W(nc_idx(1) + row_MAP_idx - 1);
    elseif strcmp(families(l), 'cycloparaffins')
        ...
        nc_idx = find(cycloparaffins.nC == nc_MAP(l));
        eta_B_star_diff = abs(cycloparaffins.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l));
        min_eta_B_star_diff = min(abs(cycloparaffins.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l)));
        row_MAP_idx = find(eta_B_star_diff == min_eta_B_star_diff, 1);

        names = 'cycloparaffins/cycloparaffins_Names.csv';
        cycloparaffins_names = readtable(names, 'VariableNamingRule', 'preserve');

        species_MAP(l) = cycloparaffins_names.Name(nc_idx(1) + row_MAP_idx - 1);
        Wi_MAP(l) = cycloparaffins.W(nc_idx(1) + row_MAP_idx - 1);
    elseif strcmp(families(l), 'dicycloparaffins')
        ...
        nc_idx = find(dicycloparaffins.nC == nc_MAP(l));
        eta_B_star_diff = abs(dicycloparaffins.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l));
        min_eta_B_star_diff = min(abs(dicycloparaffins.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l)));
        row_MAP_idx = find(eta_B_star_diff == min_eta_B_star_diff, 1);

        names = 'dicycloparaffins/dicycloparaffins_Names.csv';
        dicycloparaffins_names = readtable(names, 'VariableNamingRule', 'preserve');

        species_MAP(l) = dicycloparaffins_names.Name(nc_idx(1) + row_MAP_idx - 1);
        Wi_MAP(l) = dicycloparaffins.W(nc_idx(1) + row_MAP_idx - 1);
    elseif strcmp(families(l), 'alkylbenzenes')
        ...
        nc_idx = find(alkylbenzenes.nC == nc_MAP(l));
        eta_B_star_diff = abs(alkylbenzenes.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l));
        min_eta_B_star_diff = min(abs(alkylbenzenes.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l)));
        row_MAP_idx = find(eta_B_star_diff == min_eta_B_star_diff, 1);

        names = 'alkylbenzenes/alkylbenzenes_Names.csv';
        alkylbenzenes_names = readtable(names, 'VariableNamingRule', 'preserve');

        species_MAP(l) = alkylbenzenes_names.Name(nc_idx(1) + row_MAP_idx - 1);
        Wi_MAP(l) = alkylbenzenes.W(nc_idx(1) + row_MAP_idx - 1);
    elseif strcmp(families(l), 'alkylnaphtalenes')
        ...
        nc_idx = find(alkylnaphtalenes.nC == nc_MAP(l));
        eta_B_star_diff = abs(alkylnaphtalenes.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l));
        min_eta_B_star_diff = min(abs(alkylnaphtalenes.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l)));
        row_MAP_idx = find(eta_B_star_diff == min_eta_B_star_diff, 1);

        names = 'alkylnaphtalenes/alkylnaphtalenes_Names.csv';
        alkylnaphtalenes_names = readtable(names, 'VariableNamingRule', 'preserve');

        species_MAP(l) = alkylnaphtalenes_names.Name(nc_idx(1) + row_MAP_idx - 1);
        Wi_MAP(l) = alkylnaphtalenes.W(nc_idx(1) + row_MAP_idx - 1);
    elseif strcmp(families(l), 'cycloaromatics')
        ...
        nc_idx = find(cycloaromatics.nC == nc_MAP(l));
        eta_B_star_diff = abs(cycloaromatics.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l));
        min_eta_B_star_diff = min(abs(cycloaromatics.eta_B_star_norm(nc_idx) - eta_B_star_MAP(l)));
        row_MAP_idx = find(eta_B_star_diff == min_eta_B_star_diff, 1);

        names = 'cycloaromatics/cycloaromatics_Names.csv';
        cycloaromatics_names = readtable(names, 'VariableNamingRule', 'preserve');

        species_MAP(l) = cycloaromatics_names.Name(nc_idx(1) + row_MAP_idx - 1);
        Wi_MAP(l) = cycloaromatics.W(nc_idx(1) + row_MAP_idx - 1);
    end
end
