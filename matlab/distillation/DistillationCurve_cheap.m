...
%
% Description:
% DistillationCurve_cheap is a lightweight variant of DistillationCurve
% that stops as soon as the recovered volume fraction exceeds vol1 — the
% first experimental data point. This reduces the computational cost of
% the likelihood evaluation during the early (cheap) MCMC stage.
%
% Inputs:
...
function [volumeFraction, T_distillation] = DistillationCurve_cheap(X0, classes, index_n_eta, pressure, vol1)
...
T_distillation = nan(1, nSteps);
volumeFraction = zeros(1, nSteps);

% ---------------------------------------
% Cache all species data once
% ---------------------------------------
...
VLE_fun = @(T) Flash(T, beta, classes, index_n_eta, X, pressure);

% -----------------------------------
% Step 0: find first temperature (LLE)
% -----------------------------------
try
    [temp, ~] = fsolve(VLE_fun,T0,options);
catch
    temp = 300;
end
...
% -------------------------------------------------------
% Distillation run — early termination at vol1
% -------------------------------------------------------
for ii = 1:(nSteps-1)
    beta = 1.0 / (100.0 - mol(ii));

    [temp, ~] = fsolve(VLE_fun,temp,options);
    ...
    % Early termination: stop as soon as the first data point is covered
    if volumeFraction(ii+1) >= vol1
        break
    end
end
