...
function sobol_idx = sobol_lumped(families, classes, numComponents, chain_reshaped, model_output_samples, variable_name, pressure_distillation)
...
if strcmp(variable_name, 'deltaT_dist')
...
for p = 1:size(A_B,1)
    ...
    if strcmp(variable_name, 'deltaT_dist')
        [volumeFraction, temperature_dist] = DistillationCurve(A_B(p,1:numComponents-1), classes, index_n_eta_AB, pressure_distillation);
        T10 = interp1(volumeFraction, temperature_dist, 10, 'linear');
        T50 = interp1(volumeFraction, temperature_dist, 50, 'linear');
        T90 = interp1(volumeFraction, temperature_dist, 90, 'linear');
        y_AB(p,:) = [T50 - T10, T90 - T10];

    elseif strcmp(variable_name, 'molWeight')
