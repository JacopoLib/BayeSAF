...
[prior, likelihood, posterior] = pdf_build(fullData, families, variable_names, pressure_distillation, LowerBound_molFrac, UpperBound_molFrac, n_ranges, LowerBound_eta_B_star, UpperBound_eta_B_star);

%% --- DIFFERENTIAL EVOLUTION MARKOV CHAIN (DE-MC) TO EXPLORE POSTERIOR PDF --- %%

% +++ DE-MC parameters +++ %
maxIterations = 2000; % maximum number of iterations for each chain
...
outlier_method = 'mad'; % outlier chain detection method

[chain, posterior_pdf, chain_reshaped, posterior_pdf_reshaped, AR, R_hat, t_convergence]  = DifferentialEvolutionMarkovChain(posterior, families, LowerBound_molFrac, UpperBound_molFrac, n_ranges, LowerBound_eta_B_star, UpperBound_eta_B_star, maxIterations, scaling_factor, noise_X, noise_nc, noise_eta, N_chains, outlier_method);
...
visualizeResults(fullData, families, chain, AR, R_hat, chain_reshaped, t_convergence, maxIterations, n_ranges, numComponents, variable_names, x_MAP, eta_B_star_MAP, minT_array, maxT_array, pressure_distillation, fuel_name);