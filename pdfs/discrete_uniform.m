...
% DESCRIPTION:
% The discrete_uniform function calculates the probability of a discrete
...
% Outputs:
% 1) prob: probability density of the discrete uniform distribution
...
for i = 1:numel(n)
    if n(i) < min(n_ranges{i})
        prob = 0;
        return
    end

    if n(i) > max(n_ranges{i})
        prob = 0;
        return
    end
