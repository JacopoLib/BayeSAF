function  [prior, likelihood, posterior] = pdf_build(fullData, families, variable_names, pressure_distillation, LowerBound_x, UpperBound_x, n_ranges, LowerBound_eta_B_star, UpperBound_eta_B_star)
...
% Define a safe log function to avoid log(0)
safe_log = @(p) log(max(p, eps));

prior_informative_x = @(x)safe_log(uniform_pdf(x, LowerBound_x, UpperBound_x));

% Set alpha = 1 to get a symmetric Dirichlet distribution
alpha = 1;
prior_constraint_x = @(x)safe_log(dirichlet_pdf(x, alpha, LowerBound_x));

prior_x = @(x) prior_informative_x(x) + prior_constraint_x(x);

prior_informative_eta_B_star = @(eta_B_star)safe_log(uniform_pdf(eta_B_star, LowerBound_eta_B_star, UpperBound_eta_B_star));

prior_informative_n = @(n) safe_log(discrete_uniform(n, n_ranges));
...
for i = 1:numel(fullData)
    ...
    likelihood = @(x, n, eta_B_star) LikelihoodFunction(families, data, stdData, x, n, eta_B_star, variable_name, pressure_distillation, n_ranges);
